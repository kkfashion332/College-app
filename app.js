/* ═══════════════════════════════════════════════════════
   UNIQUE FASHION — CUSTOMER ONLY JS (PURE FRONTEND)
═══════════════════════════════════════════════════════ */
...
$("confirmOrderBtn").onclick = async () => {
  ...
  const userEmail = chkMobile + "@genzstore.local"; // Local identifier
  ...
  // Orders will now save securely and only in Local Storage
  btn.disabled = false;
  let localUserOrders = load("knk_my_orders_local", []); 
  localUserOrders.unshift(orderData); 
  save("knk_my_orders_local", localUserOrders);
...
window.renderMyOrders = function() {
  ...
  let displayOrders = load("knk_my_orders_local", []);
  ...
  displayOrders.forEach((o) => {
    ...
    html += `
    <div class="mo-card" onclick="pushModalState(); openMyOrderModal('${o.savedAt}')">
      <div class="mo-head">
...
window.openMyOrderModal = function (idStr) {
  let allSrc = load("knk_my_orders_local", []);
  const o = allSrc.find((x) => x.savedAt && x.savedAt.toString() === idStr.toString());
  if (!o) return;
